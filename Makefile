...
docs: ## Generate projects docs (from "Documentation" directory)
	...
	docker run --rm --pull always -v "$(CURDIR)":/project -t ghcr.io/typo3-documentation/render-guides:latest --config=Documentation

.PHONY: docs-fast
docs-fast: ## Generate projects docs (from "Documentation" directory)
	mkdir -p Documentation-GENERATED-temp

	docker run --rm -v "$(CURDIR)":/project -t ghcr.io/typo3-documentation/render-guides:latest --config=Documentation
